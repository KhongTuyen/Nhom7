...
namespace StationeryShop.Controllers
{
	public class ProductController : Controller
	{
		...
		public ActionResult Index(long id)
		{
			...
			ViewBag.relatedProds = db.products.Where(p => p.cat_id == prod.cat_id && p.id != id).Take(5).ToList();
			return View(prod);
		}
		...
		{
			//sỬA LỖI TÌM KIẾM KHI KHÔNG NHẬP NỘI DUNG
            if (string.IsNullOrEmpty(q))
            {
				q = normalize(q??"");
				var products = db.products.ToList().Where(p => normalize(p.name).Contains(q)).ToPagedList(page ?? 1, 15);
				ViewBag.error = true;
